...
def send_welcome(message):
    ...
    f = open(file_name, 'w')
    template = f'name,1000000,0,1,0\n' \
               f'Власов Максим,1,3,5000'
    f.write(template)
    f.close()
    print(file_name)
...
def get_text_messages(message):
    ...
    if text == 'играть🎮':
        ...
        if not None:
            name, money, trophy, number_players, increase = result
            text = f'Ваш профиль:\n\nНикнейм: {name}\nТрофеи {trophy}🏆\n' \
    ...
    if mascott != 1:
        ...
        if win == 1:
            bot.send_message(message.from_user.id, f'Твой персонаж на втором этапе! Выбери куда пнуть мяч!',
                             reply_markup=game_menu)
            _1lvl2rnd = open('1lvl2rnd.png', 'rb')
            bot.register_next_step_handler(message, second_round)
            bot.send_photo(message.from_user.id, _1lvl2rnd)
            _1lvl2rnd.close()
